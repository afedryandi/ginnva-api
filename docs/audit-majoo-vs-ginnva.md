...
| Laporan/daftar ringkas "Booking Dibatalkan" per periode | **Sudah Ada** | `VoidReport.php` (Penjualan > Laporan Penjualan > Laporan Void, dibangun 2026-09-09) — ringkasan total booking dibatalkan + potensi pendapatan hilang, tabel detail (No. Booking, Tanggal Order/Dibatalkan, Pelanggan, Toko, Layanan, Dibatalkan Oleh, Nilai Transaksi), filter tanggal + scope toko, export Excel & PDF. Dikonfirmasi lengkap 2026-09-24. |
...
| Fitur Majoo | Status | Catatan |
|---|---|---|
| **Ekspor Laporan** (ke Excel/PDF/CSV) — ada di HAMPIR SEMUA halaman laporan Majoo | **Sudah Ada — dibangun 2026-09-24** | Re-cek 2026-09-24: ternyata gap sebenarnya sudah menyempit jauh sejak temuan 2026-09-17 — 30+ Resource/Report Page sudah lebih dulu punya export (dibangun bertahap sesi-sesi sebelumnya tanpa update balik dokumen ini). Sisa 11 Report Page yang belum: Neraca, Laba Rugi, Neraca Saldo, Arus Kas, Buku Besar, Laporan Pelanggan, Laporan Keuangan, Durasi Pengerjaan (2 varian), Durasi Layanan Teknisi, Utilisasi Teknisi — SEMUA sudah dibangun (Export Excel + PDF, pola sama dgn `VoidReport`/`SalesSummaryReport`, sumber data selalu `getResult()` yang sama dgn tampilan layar, tidak ada query duplikat). `WorkScheduleCalendarReport` SENGAJA dikecualikan — itu kalender operasional klik-edit, bukan laporan tabular, export tidak relevan. Belum di-commit, menunggu review user. |
...
| Fitur Majoo | Status | Catatan |
|---|---|---|
| **Metrik Utilisasi Bay/Stall Instalasi** (jam operasional vs terpakai vs kosong vs dibatalkan, per bay) | **Sudah Ada — dibangun 2026-09-24** | Didesain lewat diskusi langsung dgn pemilik bisnis: Ginnva punya 2 ZONA fisik (bukan bay individual) — Zona Detailing & Persiapan (3 slot, mencakup Cuci/Pembersihan/Pemanasan/Detailing) & Zona Instalasi + QC (6 slot) — kapasitas beda-beda per toko. Karena durasi tiap tahap tidak bisa diestimasi (variasi ukuran/kondisi mobil), utilisasi dihitung dari DURASI NYATA hasil rekonstruksi `activity_log` perpindahan `current_stage`/`secondary_stage` (bukan asumsi rata-rata). Dibangun: migrasi 2 kolom slot baru di `stores`, `BayZoneUtilizationService` (rekonstruksi interval + sweep-line peak-concurrent), halaman "Laporan Utilisasi Zona/Bay" (grup navigasi baru "Laporan Fasilitas") + export Excel/PDF. Sengaja terpisah dari `ReservationUtilizationReport` yang sudah ada (itu 1 angka kapasitas agregat per toko, laporan ini 2 zona dgn durasi nyata) — jangan digabung. Belum di-commit, menunggu review user. |
...
| Fitur Majoo | Status | Catatan |
|---|---|---|
| Saldo prabayar/store credit customer dgn masa berlaku | **Tidak Perlu — ditunda, dikonfirmasi user 2026-09-24** | Konsep terpisah dari DP: customer beli/top-up saldo di muka (mis. paket "beli saldo Rp 5jt, dipakai kapan saja utk servis apapun"), bukan DP khusus 1 booking. Ditanyakan ke user apakah mau menawarkan skema pra-bayar/member-saldo semacam ini — jawaban: tidak perlu, ditunda dulu. TIDAK boleh disalahartikan sbg "DP" (skema Uang Muka tetap kebutuhan terpisah, prioritas tinggi, lihat temuan sebelumnya, dan tetap berjalan seperti biasa).
...
| **Toggle Batch Number PER PRODUK** (bukan global) | **Sudah Ada — dibangun 2026-09-24** | Didesain lewat diskusi workflow dgn pemilik bisnis: `ScrollCode` (kode roll fisik, per `film_product_id`) sudah lama punya traceability lengkap (`usages()`/`warranties()`), TAPI "Catat Pemakaian" (`ScrollCode::recordUsage()`) sepenuhnya independen dari siklus SPK — staff bisa tandai SPK selesai (`checked_out_at`) tanpa pernah catat roll yang dipakai, bikin recall roll cacat bisa kelewat booking yg lupa dicatat. Solusi: toggle `tracks_batch` baru di `FilmProduct` (opt-in per produk, default false) + gate baru di `SpkService::assertBatchTrackingSatisfied()` — SPK booking dgn produk `tracks_batch=true` TIDAK BISA ditandai selesai sebelum ada `ScrollCodeUsage` tertaut ke booking itu. Berlaku di Filament & mobile API (SpkController, termasuk fix bug: endpoint update SPK sebelumnya tidak menangkap RuntimeException sama sekali, sudah diperbaiki). Belum di-commit, menunggu review user. |
...
| Modal "Ubah Multiprice" → tabel harga riil + fitur "Preset Harga Produk" (bulk Markup/Markdown %/Rp ke banyak produk sekaligus, atau "Reset dan tentukan manual") | **Tidak Perlu — ditunda, dikonfirmasi user 2026-09-24** | Ditanyakan ke user apakah ada kebutuhan update harga massal (mis. semua produk PPF naik 5% sekaligus) — jawaban: tidak perlu, ditunda dulu, update harga tetap satu-satu seperti sekarang. |
...
| Fitur Majoo | Status | Catatan |
|---|---|---|
| **Tipe produk "Layanan" berbasis Durasi+Sesi (terpisah dari Barang)** | **Sudah Ada — dikonfirmasi 2026-09-24** | Dikonfirmasi user: Ginnva memang jual jasa berdiri sendiri (Detailing, Washing) tanpa produk film. Ternyata SUDAH terpenuhi sejak 2026-09-10/2026-09-22 lewat `FilmProduct.product_type` ('detailing'/'premium_wash', SKU `SVC-DETAILING`/`SVC-PREMIUM-WASH`) + `Booking.product_detailing`/`product_premium_wash` — booking bisa dibuat murni jasa tanpa pilih varian PPF/Kaca Film sama sekali. Field "Durasi" eksplisit Majoo tidak ada, tapi durasi NYATA per job sudah tercatat via `Spk.checked_in_at`/`checked_out_at` (dipakai laporan Durasi Pengerjaan, f11/f12/f13) — cukup sebagai basis data, tidak perlu field durasi estimasi terpisah di produk. Tidak ada kode yang perlu dibangun. |
...
| Fitur Majoo | Status | Catatan |
|---|---|---|
| **Komisi Bertingkat berbasis Durasi Layanan Selesai** — target komisi progresif utk teknisi/installer berdasar akumulasi durasi servis per periode | **Ditunda — user belum bisa tentukan skema, 2026-09-24** | Blocker data sudah tidak ada (durasi nyata sudah tercatat via `Spk.checked_in_at`/`checked_out_at`, lihat f33) — murni menunggu keputusan skema insentif. Ditawarkan 2 contoh (A: tarif progresif per-jam berjenjang mirip pajak, B: bonus flat per ambang batas jam), user belum bisa putuskan. Tanya lagi kalau owner sudah siap dgn skema konkret (basis periode, struktur tingkatan, per-individu vs per-tim, pengganti/tambahan dari `Technician.commission_amount` yg sudah ada). |
...
| "Pola Kustom" (Kustom Jadwal Kerja) — kemungkinan siklus non-mingguan (shift bergilir/rolling) | **Tidak Perlu — dikonfirmasi user 2026-09-24** | Ditanyakan langsung ke user: Ginnva tidak punya kebutuhan shift bergilir/rolling (semua staff pakai jadwal fixed mingguan). `WorkScheduleResource` yang sudah dibangun 2026-09-22 (Pola Standar 7-hari + override 1-hari) sudah cukup, tidak perlu ditiru. |
...
| Fitur Majoo | Status | Catatan |
|---|---|---|
| Master Tingkat Jabatan dgn hierarki eksplisit berjenjang (tiap level tau siapa atasannya) | **Tidak Perlu — ditunda, dikonfirmasi user 2026-09-24** | Dicek: `User.php` Ginnva memang belum punya entitas "Jabatan"/"Departemen" formal (cuma role akses `hasMenuAccess()` + `store_id`, lihat komentar di `User.php` soal Riwayat Karir). Ditanyakan ke user apakah perlu dibangun — jawaban: tidak perlu, ditunda dulu. Tidak ada kebutuhan konkret saat ini (approval berjenjang keuangan yang sudah ada tetap pakai role akses langsung, bukan hierarki jabatan). |
